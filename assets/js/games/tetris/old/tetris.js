...
class Tetris {
  ...
  resetCurrentPiece() {
    this.currentPiece = new Tetroid(this.nextPiece.matrix, this.nextPiece.topLeftPos);
    this.currentPiece.topLeftPos.y = 0;
    this.currentPiece.topLeftPos.x = (this.arena[0].length / 2 | 0) -
  ...
  createNewTetroid() {
    ...
    return new Tetroid(tetroidFromBank.matrix, tetroidFromBank.topLeftPos);
  }
  ...
  saveCurrentPieceIntoArena() {
    this.currentPiece.matrix.forEach((row, y) => {
      row.forEach((value, x) => {
        if (value !== 0) {
          this.arena[y + this.currentPiece.topLeftPos.y][x + this.currentPiece.topLeftPos.x] = value;
        }
      });
    });
  }
  ...
  drawAll() {
    this.drawMainBoard();
    this.drawArena();
    this.drawMatrix(this.mainCanvasContext, this.currentPiece.matrix, this.currentPiece.topLeftPos);
  }
  ...
  drawSecondaryBoard() {
    ...
    this.drawMatrix(this.smallCanvasContext, this.nextPiece.matrix, this.nextPiece.topLeftPos);
